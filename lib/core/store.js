...
export const rehydratedSelector = createSelector([rehydrationSeletor], rehydration => rehydration.isRehydrated);
...
export const buildStore = (resources, { logger = false, persist = true }) => {
  ...
  const enhancers = persist ? compose(applyMiddleware(...middleware), autoRehydrate()) : compose(applyMiddleware(...middleware));
