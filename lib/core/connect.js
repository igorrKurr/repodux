    ...
    var WithResource = function (_Component) {
      ...
      _createClass(WithResource, [{
        ...
        value: function render() {
          ...
          var selectedProps = resource.selectors;
          if (only) {
            selectedProps = only.reduce(function (acc, p) {
              var selector = resource.selectors[p];
              if (selector) {
                return _extends({}, acc, _defineProperty({}, p, selector));
              } else {
                return acc;
              }
            }, {});
          ...
          var selectedActions = resource.actions;
          if (actions) {
            selectedActions = actions.reduce(function (acc, p) {
              var action = resource.actions[p];
              if (action) {
                return _extends({}, acc, _defineProperty({}, p, action));
              } else {
                return acc;
              }
            }, {});
          ...
          mapStateToProps = (0, _reselect.createStructuredSelector)(selectedProps);
          ...
          var mergeProps = function mergeProps(stateProps, dispatchProps, ownProps) {
            return _extends(_defineProperty({}, resource.name, _extends({}, stateProps, dispatchProps, {
              error: _extends({
